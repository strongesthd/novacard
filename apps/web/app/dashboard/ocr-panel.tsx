...
export default function OcrPanel() {
  ...
  const readOptimizedImage = (file: File) => new Promise<string>((resolve, reject) => {
    ...
    reader.onload = () => { const image = new Image(); image.onload = () => { const scale = Math.min(1, 1000 / Math.max(image.width, image.height)); const canvas = document.createElement("canvas"); canvas.width = Math.max(1, Math.round(image.width * scale)); canvas.height = Math.max(1, Math.round(image.height * scale)); const context = canvas.getContext("2d"); if (!context) return reject(new Error("Kh?ng th? x? l? ?nh")); context.drawImage(image, 0, 0, canvas.width, canvas.height); resolve(canvas.toDataURL("image/jpeg", 0.65)); }; image.onerror = () => reject(new Error("Kh?ng ??c ???c ?nh")); image.src = String(reader.result); }; reader.onerror = () => reject(new Error("Kh?ng ??c ???c ?nh")); reader.readAsDataURL(file);
  });
